...
class FaceRequestInfo extends StatefulWidget {
  @override
  _FaceRequestInfoState createState() => _FaceRequestInfoState();
}

class _FaceRequestInfoState extends State<FaceRequestInfo> {
  File _imageFile;
  Image _imageUser ;
  var encode = "";
  String title = "";
  ...
  Future<void> getInfo(BuildContext context)async{
    ...
    userRegis.getRequestStaffInfo(request, globals.TOKEN).then((value){
          ...
          if(response.respCode==ResponseCode.APPROVED){
            setState(() {
              ...
              switch(response.faceRegisStatus){
                //static const FACE_REGISTER_REQUEST = "6013";
                case ResponseCode.FACE_REGISTER_REQUEST: facestatus..text = "Request face register.";
                break;
                ...
                break;
                case ResponseCode.FACE_UPDATE_REQUEST: facestatus..text = "Request face update";
                break;
    ...
    faceConnection.faceExtraction(request,globals.TOKEN).then((value) {
      ...
      if (value.statusCode == 200) {
        ...
        if (reqRefNo == response.reqRefNo &&
          ...
          title = "Success";
          dialogContentSuccess(title,description,'/request_update');
  ...
  Future<void> faceDeclined(BuildContext context) async {
    String reqRefNo = 'REQ' + randReqRef.generateREQRefNo();
    String declined ='';
    FaceUpdateStatusRequest request;
    if(faceRegisStatus==ResponseCode.FACE_UPDATE_REQUEST){
      request = FaceUpdateStatusRequest(employeeNo, image, ResponseCode.FACE_UPDATE_DECLINED, reqRefNo);
    }else
      {
        request = FaceUpdateStatusRequest(employeeNo, image, ResponseCode.FACE_REGISTER_DECLINED, reqRefNo);
      }
    print("Tokens:"+globals.TOKEN);
                ...
                child: Padding(
                  ...
                  child: ListView(
                    children: <Widget>[
                      ...
                      Container(
                        width: width,
                        child: _textFieldStatus(facestatus, "Request"),
                      ),
                      Row(
